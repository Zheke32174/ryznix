...
echo "===== redeploy slinux-install.py (efficient: per-package new-file convert + auto shebang) ====="
...
BINDIRS=("usr/bin","bin","usr/sbin","sbin","usr/games","usr/local/bin")
SHEBANG_PREFIXES=("/usr/bin/","/bin/","/usr/sbin/","/sbin/","/usr/local/bin/")

def run(args, cwd=None):
    return subprocess.run(args, cwd=cwd, env=ENV, stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
def curl(url, out):
...
def convert_and_fix(paths):
    ...
    for p in paths:
        ...
        elif head==b"#!":   # script — rewrite absolute interpreter to rootfs
...
def main():
    ...
    conv,sh=convert_and_fix(allp)
    print(f"[convert] {conv} ELF + {sh} shebang fixes over {len(allp)} new files")
    print("[done]")
    return 0
sys.exit(main())
PYEOF
echo "deployed ($($BB wc -l < $T/slinux-install.py) lines)"

echo "===== write super-linux profile + wire into superlinux.sh ====="
cat > "$U/super-profile.sh" <<'PROF'
# super-linux runtime env (Ubuntu glibc primary). Sourced by superlinux.sh.
U=/data/local/tmp/ubuntu; T=/data/local/tmp
export LANG=C.UTF-8 LC_ALL=C.UTF-8 TERM=xterm-256color
export PERL5LIB=$U/usr/share/perl/5.38:$U/usr/share/perl5:$U/usr/lib/aarch64-linux-gnu/perl5/5.38:$U/usr/lib/aarch64-linux-gnu/perl-base
export COWPATH=$U/usr/share/cowsay/cows
export TERMINFO=$U/usr/share/terminfo TERMINFO_DIRS=$U/usr/share/terminfo:$U/lib/terminfo
export MANPATH=$U/usr/share/man
export PYTHONHOME=$U/usr PYTHONDONTWRITEBYTECODE=1
[ -f $U/etc/ssl/certs/ca-certificates.crt ] && export SSL_CERT_FILE=$U/etc/ssl/certs/ca-certificates.crt CURL_CA_BUNDLE=$U/etc/ssl/certs/ca-certificates.crt
PROF
# add a line to superlinux.sh to source the profile (idempotent)
$BB grep -q super-profile.sh "$T/superlinux.sh" || $BB sed -i 's#^exec \$U/bin/bash#[ -f $U/super-profile.sh ] \&\& . $U/super-profile.sh\nexec $U/bin/bash#' "$T/superlinux.sh"
echo "profile wired: $($BB grep -c super-profile.sh $T/superlinux.sh)"
echo "===== DONE ====="
